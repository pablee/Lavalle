<?php

echo '<div class="row contenido">';

    //Filtro menu-izq
    echo
    '
    <div class="col-md-3 contenido filtro">
        <table class="table">
            <thead>
             <tr>
              <th>Filtrar por</th>
             </tr>
            </thead>

            <tbody class="text-center">
             <tr>
              <td>Marca</td>
             </tr>
             <tr>
              <td>Modelo</td>
             </tr>
             <tr>
              <td>Color</td>
             </tr>
             <tr>
              <td>Talle</td>
             </tr>
             <tr>
              <td>Precio</td>
             </tr>
            </tbody>
        </table>
    </div>
    ';

    //Productos
    echo'
    <div class="col-md-9 contenido">

        <div class="filtro-barra" id="filtro-barra">
            Cascos> Abiertos> Shoei
        </div>

        <div class="col-md-3">
            <div class="thumbnail thumbnail-producto">
                <img src="img/productos/casco_abierto.jpg" alt="baul" height="auto" width="auto">
                <div class="caption">
                    <div class="text-center">
                        <a href="#" target="_blank">CASCO BLAUER</a>
                        <p class="descripcion">Pilot 1.1 Graphic B White</p>
                        <p><strike>$2000</strike> $18499 <span class="label label-danger">10% OFF</span></p>
                    </div>
                </div>
            </div>
        </div>

        <div class="col-md-3">
            <div class="thumbnail thumbnail-producto">
                <img src="img/productos/casco_abierto.jpg" alt="baul" height="auto" width="auto">
                <div class="caption">
                    <div class="text-center">
                        <a href="#" target="_blank">CASCO BLAUER</a>
                        <p class="descripcion">Pilot 1.1 Graphic B White</p>
                        <p><strike>$2000</strike> $18499 <span class="label label-danger">10% OFF</span></p>
                    </div>
                </div>
            </div>
        </div>

        <div class="col-md-3">
            <div class="thumbnail thumbnail-producto">
                <img src="img/productos/casco_abierto.jpg" alt="baul" height="auto" width="auto">
                <div class="caption">
                    <div class="text-center">
                        <a href="#" target="_blank">CASCO BLAUER</a>
                        <p class="descripcion">Pilot 1.1 Graphic B White</p>
                        <p><strike>$2000</strike> $18499 <span class="label label-danger">10% OFF</span></p>
                    </div>
                </div>
            </div>
        </div>
    </div>
    ';

echo '</div>'
?>