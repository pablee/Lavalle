    <!--Barra de navegacion-->
    <nav class="navbar navbar-inverse navbar-static-top">
        <div class="container text-center">
            <ul class="nav navbar-nav">
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Motos</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Cuatriciclos</a></li>
                                <li><a href="#">Cubs</a></li>
                                <li><a href="#">Custom</a></li>
                                <li><a href="#">Enduro</a></li>
                                <li><a href="#">Enduro - Calle</a></li>
                                <li><a href="#">Scooters</a></li>
                                <li><a href="#">Streets</a></li>
                                <li><a href="#">Utilitarios</a></li>
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Alta gama</b></li>
                                <li><a href="#">Yamaha</a></li>
                                <li><a href="#">Kawasaki</a></li>
                                <li><a href="#">Suzuki</a></li>
                                <li><a href="#">Benelli</a></li>
                            </div>
                            <div class="col-md-7 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_moto.png" alt="casco" width="300" height="300">
                            </div>
                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Cascos</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Abiertos</a></li>
                                <li><a href="#">Motocross</a></li>
                                <li><a href="#">Integrales</a></li>
                                <li><a href="#">Rebatibles</a></li>
                                <li><a href="#">Modulares</a></li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Acerbis</li>
                                <li>Agv</li>
                                <li>Airoh</li>
                                <li>Arai</li>
                                <li>Benelli</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Beon</li>
                                <li>Fox</li>
                                <li>Givi</li>
                                <li>Hjc</li>
                                <li>Ls2</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Momo</li>
                                <li>Nzi</li>
                                <li>Origine</li>
                                <li>Peels</li>
                                <li>Schuberth</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Shiro</li>
                                <li>Shoei</li>
                                <li>V-can</li>
                            </div>
                            <div class="col-md-5 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_casco.png" alt="casco" width="300" height="300">
                            </div>

                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Indumentaria</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Pista</a></li>
                                <li><a href="#">Touring</a></li>
                                <li><a href="#">Enduro-cross</a></li>
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Acerbis</li>
                                <li>Fox</li>
                                <li>Joe rocket</li>
                                <li>Motorman</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Octane</li>
                                <li>Rpmcroos</li>
                                <li>Shift</li>
                                <li>Yamaha</li>
                            </div>

                            <div class="col-md-6 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_indumentaria.jpg" alt="casco" width="300" height="300">
                            </div>

                        </div>
                    </ul>

                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Calzado</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Pista</a></li>
                                <li><a href="#">Motocross</a></li>
                                <li><a href="#">Touring</a></li>
                                <li><a href="#">Enduro-cross</a></li>
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Acerbis</li>
                                <li>Cros Roads</li>
                                <li>Forma</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Fox</li>
                                <li>Gaerne</li>
                                <li>Sidi</li>
                            </div>
                            <div class="col-md-6 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_botas.png" alt="casco" width="300" height="300">
                            </div>

                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Accesorios</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Antiparras</a></li>
                                <li><a href="#">Bauleras</a></li>
                                <li><a href="#">Intercomunicadores</a></li>
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Coocase</li>
                                <li>Ethen</li>
                                <li>Fox</li>
                                <li>Givi</li>
                                <li>Nenki</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Progrip</li>
                                <li>Sena</li>
                                <li>Shad</li>
                                <li>Thor</li>
                                <li>Vega</li>
                            </div>
                            <div class="col-md-6 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_baul.png" alt="casco" width="300" height="300">
                            </div>

                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Cubiertas</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Motocross</a></li>
                                <li><a href="#">Cuatri</a></li>
                                <li><a href="#">Enduro calle</a></li>
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Continental</li>
                                <li>Dunlop</li>
                                <li>Metzeler</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Pirelli</li>
                                <li>Rinaldi</li>
                                <li>Torc</li>
                            </div>
                            <div class="col-md-6 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_accesorios.jpg" alt="casco" width="300" height="300">
                            </div>
                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Repuestos</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Baterias</a></li>
                                <li><a href="#">Escapes</a></li>
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Cott</li>
                                <li>Dinavolt</li>
                                <li>Dirt Race</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Maxim</li>
                                <li>Outdo</li>
                                <li>R72</li>
                            </div>
                            <div class="col-md-6 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_casco.jpg" alt="casco" width="300" height="300">
                            </div>

                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Seguridad</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Alarmas</a></li>
                                <li><a href="#">Candados</a></li>
                                <li><a href="#">Lingas</a></li>
                                <li><a href="#">Trabadiscos</a></li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Kovix</li>
                                <li>Plaza</li>
                                <li>Positron</li>
                            </div>

                            <div class="col-md-8 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_casco.jpg" alt="casco" width="300" height="300">
                            </div>
                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Protecciones</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Coderas</a></li>
                                <li><a href="#">Integrales</a></li>
                                <li><a href="#">Pecheras</a></li>
                                <li><a href="#">Rodilleras</a></li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Acerbis</li>
                                <li>Fox</li>
                                <li>Shift</li>
                            </div>
                            <div class="col-md-8 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_casco.jpg" alt="casco" width="300" height="300">
                            </div>
                        </div>
                    </ul>
                </li>
                <li class="dropdown">
                    <a href="#" class="dropdown-toggle" data-toggle="dropdown">Conocenos</a>
                    <ul class="dropdown-menu" role="menu">
                        <div class="row">
                            <div class="col-md-1 margin-top">
                            </div>
                            <div class="col-md-2 margin-top">
                                <li><b>Tipo</b></li>
                                <li><a href="#">Pista</a></li>
                                <li><a href="#">Motocross</a></li>
                                <li><a href="#">Touring</a></li>
                                <li><a href="#">Enduro-cross</a></li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <li><b>Marcas</b></li>
                                <li>Acerbis</li>
                                <li>Cros Roads</li>
                                <li>Forma</li>
                            </div>
                            <div class="col-md-1 margin-top">
                                <br>
                                <li>Fox</li>
                                <li>Gaerne</li>
                                <li>Sidi</li>
                            </div>
                            <div class="col-md-7 text-right">
                                <img src="<?php echo base_url(); ?>assets/img/categorias/cat_casco.jpg" alt="casco" width="300" height="300">
                            </div>

                        </div>
                    </ul>
                </li>
            </ul>
        </div>
    </nav>
